...
export function KpiCard({ label, value, delta, deltaUp, highlight }: KpiCardProps) {
  return (
    <div style={{
      background: highlight ? 'linear-gradient(135deg, #10b98111, #111)' : '#111',
      border: `1px solid ${highlight ? '#10b981' : '#222'}`,
      ...
      <span style={{ fontSize: 26, fontWeight: 700, color: highlight ? '#10b981' : '#f0f0f0' }}>
        {value}
      </span>
      {delta && (
        <span style={{ fontSize: 11, color: deltaUp ? '#10b981' : '#ef5350' }}>
          {deltaUp ? '▲' : '▼'} {delta}
        </span>
      )}
    </div>
